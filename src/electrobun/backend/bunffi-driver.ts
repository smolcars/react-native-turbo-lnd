...
const LND_DLL_FILENAME = `liblnd.${suffix}`;
...
> = {
  ...
  [K in UnaryMethod | ServerStreamMethod]: UnaryOrServerSignature;
} & {
  [K in BidiStreamMethod]: BidiSignature;
};

type BunffiDriverResult<
  UnaryMethod extends string,
  ServerStreamMethod extends string,
  BidiStreamMethod extends string,
> = {
...
export function createBunffiDriver<
  UnaryMethod extends string,
  ServerStreamMethod extends string,
  BidiStreamMethod extends string,
>({
  ...
  serverStreamMethods,
  bidiStreamMethods,
}: CreateBunffiDriverParams<
  UnaryMethod,
  ServerStreamMethod,
  BidiStreamMethod
>): BunffiDriverResult<UnaryMethod, ServerStreamMethod, BidiStreamMethod> {
  type Symbols = FfiSymbols<UnaryMethod, ServerStreamMethod, BidiStreamMethod>;
  type LndLibrary = ReturnType<typeof dlopen<Symbols>>;
  ...
  const ffiSignatures = {
    ...
    ...Object.fromEntries(
      [...unaryMethods, ...serverStreamMethods].map((method) => [
        method,
        FFI_SIGNATURE_UNARY_OR_SERVER,
      ])
    ),
    ...Object.fromEntries(
      bidiStreamMethods.map((method) => [method, FFI_SIGNATURE_BIDI])
    ),
  };
  ...
  if (dllResolution.path === null) {
    throw new Error(
      [
        "Unable to find the liblnd shared library.",
        `cwd=${process.cwd()}`,
  ...
  const resolvedDllPath = dllResolution.path;
  const lnd: LndLibrary = dlopen<Symbols>(
    resolvedDllPath,
    ffiSignatures as any
  );
  const symbols = lnd.symbols as any;
  ...
  async function start(args: string): Promise<string> {
    ...
    startPromise = new Promise<string>((resolve, reject) => {
      ...
      symbols.start(argsBytes, ptr(callbackStruct));
    }).finally(() => {
      startPromise = null;
    });
  ...
  function openServerStream(
    ...
    symbols[method](requestBuffer, requestPayload.length, ptr(callbackStruct));
  ...
  function openBidiStream(
    ...
    const streamPtr = symbols[method](ptr(callbackStruct));
    ...
    return {
      send(dataB64: ProtobufBase64): boolean {
        ...
        try {
          ...
          return (
            symbols.SendStreamC(streamPtr, requestBuffer, payload.length) === 0
          );
        } catch (error) {
          const message =
      ...
      stop(): boolean {
        ...
        let stopped = false;
        try {
          stopped = symbols.StopStreamC(streamPtr) === 0;
        } catch (error) {
          cleanup();
          const message =
            error instanceof Error ? error.message : String(error);
