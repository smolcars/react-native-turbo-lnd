...
namespace winrt::ReactNativeTurboLndExample {
...
#if __has_include("codegen/NativeExampleAppPathsSpec.g.h")
  using ModuleSpec = ReactNativeTurboLndExampleCodegen::ExampleAppPathsSpec;
#endif

  REACT_SYNC_METHOD(getLndDirectory)
  std::string getLndDirectory() noexcept {
    try {
      auto directory =
      ...
      auto directory = std::filesystem::temp_directory_path() / "react-native-turbo-lnd" / "lnd";
      std::error_code ec;
